...
class HandlersTestCase(TestCase):
    ...
    def test_post_prompt_handler_success(self, mock_celery_delay, mock_handle_run_creation, mock_validate_request):
    ...
    def test_post_prompt_handler_validation_failure(self, mock_validate_request):
        mock_validate_request.return_value = JsonResponse({"error": "invalid form"}, status=400)
    ...
    @patch('task_ai.app.handlers.handle_run_creation')
    @patch('task_ai.app.handlers.periodically_check_run_status.delay')
    def test_post_prompt_handler_exception(self, mock_celery_delay, mock_handle_run_creation, mock_validate_request):
        mock_validate_request.return_value = None
    ...
    @patch('task_ai.app.handlers.validate_request')
    def test_get_prompt_handler_success(self, mock_validate_request, mock_messages_list):
        mock_validate_request.return_value = None
    ...
    def test_get_prompt_handler_unsupported_type(self):
        ...
        self.assertIsInstance(response, JsonResponse)
        self.assertEqual(response.status_code, 400)
        self.assertIn('Select a valid choice. unsupported is not one of the available choices.', str(response.content))
